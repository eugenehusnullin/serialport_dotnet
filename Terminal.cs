...
namespace SerialPortTerminal
{
    ...
    public partial class frmTerminal : Form
    {
        ...
        private bool startedCAN = false;
        private Thread threadCAN;
        #endregion
        ...
        private void SaveSettings()
        {
            ...
            settings.OnPressSaveToFile = chbChooseSave.Checked;
            settings.SendFileName = tbSendFileName.Text;
        ...
        private void InitializeControlValues()
        {
            ...
            chbChooseSave.Checked = settings.OnPressSaveToFile;
            tbSendFileName.Text = settings.SendFileName;
        ...
        private void EnableControls()
        {
            ...
            txtSendData.Enabled = btnSend.Enabled = btnSendFile.Enabled = btnStartStopCAN.Enabled = comport.IsOpen;
        ...
        private void lnkAbout_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            ...
            //    port_DataReceived(new object(), new object() as SerialDataReceivedEventArgs);
        }
        ...
        private byte[] accumbytes;
        private delegate void addbuttondelegate(byte[] buffer);
        private void addbutton(byte[] buffer)
        {
            if (flPanel.InvokeRequired)
            {
                flPanel.Invoke(new addbuttondelegate(addbutton), new object[] { buffer });
            }
            else
            {
                if (buffer[0] == 0xA && buffer[1] == 0xA)
                {
                    accumbytes = new byte[buffer.Length];
                    Buffer.BlockCopy(buffer, 0, accumbytes, 0, buffer.Length);
                    return;
                }
                else
                {
                    var bytes = new byte[(accumbytes == null ? 0 : accumbytes.Length) + buffer.Length];
                    if (accumbytes != null)
                    {
                        Buffer.BlockCopy(accumbytes, 0, bytes, 0, accumbytes.Length);
                    }
                    Buffer.BlockCopy(buffer, 0, bytes, accumbytes == null ? 0 : accumbytes.Length, buffer.Length);
                    accumbytes = new byte[bytes.Length];
                    Buffer.BlockCopy(bytes, 0, accumbytes, 0, bytes.Length);

                    if (//buffer[buffer.Length - 1] == 0x0 && buffer[buffer.Length - 2] == 0x6 &&
                        buffer[buffer.Length - 2] == 0xA && buffer[buffer.Length - 3] == 0xD)
                    {
                        accumbytes = null;
        ...
        void button_Click(object sender, EventArgs e)
        {
            ...
            var buffer = (byte[])button.Tag;

            if (chbChooseSave.Checked)
            {
                SaveFileDialog saveFileDialog1 = new SaveFileDialog();
                //saveFileDialog1.Filter = "*";
                saveFileDialog1.Title = "Save bytes";
        ...
        private void btnChooseFile_Click(object sender, EventArgs e)
        {
            ...
            {
                tbSendFileName.Text = openFileDialog1.FileName;
            }
        }

        private void btnSendFile_Click(object sender, EventArgs e)
        {
            if (!String.IsNullOrEmpty(tbSendFileName.Text))
            {
                System.IO.FileStream fs = new FileStream(tbSendFileName.Text, FileMode.Open);
                byte[] buffer = new byte[fs.Length];
                var readed = fs.Read(buffer, 0, (int)fs.Length);
                fs.Close();
                comport.Write(buffer, 0, readed);
            }
        }

        private void btnSaveCheckSum_Click(object sender, EventArgs e)
        {
            if (!String.IsNullOrEmpty(tbSendFileName.Text))
            {
                System.IO.FileStream fs = new FileStream(tbSendFileName.Text, FileMode.Open);
                ...
                MessageBox.Show(checkSum.ToString("X2"), "Check sum");
            }
        }
        ...
        {
            // $GPRMC,200035.000,A,5542.2686,N,3740.1003,E,0.00,0.00,070513,0,N,A*2A
            string nmea = "GPRMC,200035.000,A,5542.2686,N,3740.1003,E,0.00,0.00,070513,0,N,A";
            var bytes = Encoding.ASCII.GetBytes(nmea);

            System.IO.FileStream fs = new FileStream(tbSendFileName.Text, FileMode.Create);
            fs.Write(bytes, 0, bytes.Length);
            fs.Close();
        }

        private void tbFuelValue_TextChanged(object sender, EventArgs e)
        {
            tbRamka.Text = tbFuelValue.Text + " Z0560";
        }

        private void btnStartStopCAN_Click(object sender, EventArgs e)
        {
            startedCAN = !startedCAN;

            if (startedCAN)
            {
                threadCAN = new Thread(new ThreadStart(sendCAN));
                threadCAN.Start();
                btnStartStopCAN.Text = "Стоп";
            }
            else
            {
                threadCAN.Join();
                btnStartStopCAN.Text = "Старт";
            }
        }

        private void sendCAN()
        {
            string msg = tbRamka.Text + "\r\n";
